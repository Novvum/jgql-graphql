...
PROJECT=${NOW_PROJECT:-jgql-graphql-staging}
ALIAS=${NOW_ALIAS:-jgql-graphql-staging.now.sh}

export PATH="./node_modules/.bin:$PATH"
# 1. Wait for deployment ready
now --target production -A now.$STAGE.json -e STAGE=$STAGE -e --token "$NOW_TOKEN" --scope $TEAM
